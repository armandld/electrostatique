...
const double rho_0_tilde = 1e4;

// Résolution d'un système d'équations linéaires par élimination de Gauss-Jordan
template<class T>
vector<T> solve(const vector<T>& diag, const vector<T>& lower, const vector<T>& upper, const vector<T>& rhs) {
...
double epsilon(double r, double epsilon_a, double epsilon_b, double r1) {
    if (r < r1)
        return epsilon_a;
    else
        return epsilon_b;
...
// Fonction pour la densité de charge libre divisée par la permittivité du vide
double rho_epsilon(double r, double r1, bool uniform) {
    if (uniform)
		return 1.0;
	else {
		if (r < r1)
			return rho_0_tilde*sin(PI*r/r1);
		else
			return 0.0;
	}
...
int main(int argc, char* argv[]) {
    ...
    for (int k = 0; k < pointCount - 1; ++k) {
		...
		if (k==0) {
			...
			upper[k]=-1.0/h[k];
			diagonal[k]=1.0/h[k]+1.0/h[k-1];
			double rho_eps_k_moins = rho_epsilon(midPoint[k], r1, uniform_rho_case);
			double rho_eps_k_plus = rho_epsilon(midPoint[k-1], r1, uniform_rho_case);	
			rhs[k]=0.5*h[k-1]*rho_eps_k_moins+0.5*h[k]*rho_eps_k_plus;
		}
		lower[k]=-1.0/h[k];
    ...
    upper[upper.size() - 1] = -1.0/h[upper.size() - 1];
    ...
    for (int i = 0; i < E.size(); ++i) {
        ...
        D[i] = epsilon(midPoint[i], epsilon_a, epsilon_b, r1) * E[i];
    }
    ...
    // Export data
    {
        ...
        if (r.size() != phi.size())
            throw std::runtime_error("error when writing potential: r and phi do not have the same size");
...

    {
        ...
        if (r.size() != (E.size() + 1))
            throw std::runtime_error("error when writing electric field: size of E should be 1 less than r");
    ...
    }
    {
        ...
        if (E.size() != D.size())
            throw std::runtime_error("error when writing displacement field: size of D should be equal to E");
